...
import Snow from './artifacts/contracts/Snow.sol/Snow.json';
import './App.css';
...
const GGaddress = "0x7de7cea6f6e989197c86789f43986520f8a90f75";
...
function App() {
  ...
  async function fetchData() {
    if(typeof window.ethereum !== 'undefined') {
      ...
      const contract = new ethers.Contract(GGaddress, Snow.abi, provider);
      try {
  ...
  async function mint() {
    if(typeof window.ethereum !== 'undefined') {
      ...
      const contract = new ethers.Contract(GGaddress, Snow.abi, signer);
      try {
        let overrides = {
          from: accounts[0],
          value: data.cost
